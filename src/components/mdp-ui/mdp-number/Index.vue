<template>    
  <span style="width:100%;"> 
    <el-input-number v-if="showStyle=='origin'||showStyle==''||!showStyle" v-model="myVal" :disabled="disabled" :placeholder="placeholder" :min="min" :max="max" :controls="controls" :step="step" :step-strictly="stepStrictly"  :precision="precision" :controls-position="controlsPosition" @change="onChange"/>
              ...
              <span class="field-info"  :class="{disabled:disabled===true,enabled:disabled!==true}"> 
                ...
                <slot name="avater" :value="myVal" :field="{label:label,color:color,icon:icon,disabled:disabled,clearable:clearable}" >
                     <el-avatar   v-if="showStyle=='x'" :class="{'field-avater':true,'dashed-circle':avaterCpd.isNull,disabled:disabled===true,enabled:disabled!==true}" :icon="avaterCpd.icon" :style="{backgroundColor:avaterCpd.color}">{{formatImgFront(avaterCpd.innerText)}}</el-avatar>  
                 </slot>
                <slot name="info">
              ...
              <span class="field-oper" ref="operRef"  :class="{disabled:disabled===true,enabled:disabled!==true}"> 
                      <slot name="oper"   :value="myVal" :field="{label:label,color:color,icon:icon,disabled:disabled,clearable:clearable}">
                        <el-input-number v-model="myVal" :disabled="disabled" :placeholder="placeholder" :min="min" :max="max" :controls="controls" :step="step" :step-strictly="stepStrictly"  :precision="precision" :controls-position="controlsPosition" @change="onChange"/>
                      </slot>  
              </span>  
