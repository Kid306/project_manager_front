	...
	<section class="padding">
			...
			<el-form :model="editForm"  label-width="120px" :rules="editFormRules" ref="editForm" > 
				<el-row> 
					<el-col :span="12">
						<el-form-item label="字典代码" prop="itemCode" :rules="[{required:true,message:'请输入字典代码'}]">
							<el-input v-model="editForm.itemCode" placeholder="小写,下横线分割" :disabled="disField"/>
							<font color="blue">小写,下横线分割，例子: user_address</font>
						</el-form-item> 
					</el-col>
					<el-col :span="12">
				<el-form-item label="字典名称" prop="itemName" :rules="[{required:true,message:'请输入字典名称'}]">
					<el-input v-model="editForm.itemName" placeholder="名称" :disabled="disField"/>
				</el-form-item>  
				</el-col>
				</el-row>
				<el-row> 
				...
				<el-form-item label="分类" prop="categoryId" :rules="[{required:true,message:'请选择分类'}]">  
					<mdp-select v-model="editForm.categoryId" item-code="categoryId" :disabled="disField||!!formData.categoryId"/> 
				</el-form-item> 
					</el-col>
					<el-col :span="12">
				<el-form-item label="输入类型" prop="itemType" :rules="[{required:true,message:'请选择类型'}]"> 
					   <mdp-select v-model="editForm.itemType" item-code="item_type" :filter-fun="itemTypeFilterFun"/>
				</el-form-item> 
				</el-col>
				</el-row>  
				<el-form-item v-if="editForm.itemType=='2'" label="默认值" prop="dvalues">
					<el-input type="number"   v-model="editForm.dvalues"  placeholder="请输入默认值" :disabled="disField"/>  
				</el-form-item>  
				<el-form-item v-else-if="editForm.itemType=='3'" label="默认日期" prop="dvalues">
					<el-date-picker type="date" value-format="YYYY-MM-DD HH:mm:ss" format="YYYY-MM-DD"  v-model="editForm.dvalues"  placeholder="请输入默认日期值" :disabled="disField"/>  
				</el-form-item>  
				<el-form-item v-else-if="editForm.itemType=='4'" label="默认值" prop="dvalues">
					<mdp-select v-model="editForm.dvalues" :item-code="editForm.itemCode" placeholder="请选择" :disabled="disField"/>
				</el-form-item> 
				...
				<el-form-item v-else-if="editForm.itemType=='6'" label="默认文件链接" prop="dvalues"> 
					<el-input v-model="editForm.dvalues" style="width:40%;" placeholder="文件链接地址" :disabled="disField"/> 
					<el-input v-model="editForm.dnames"  style="width:40%;" placeholder="文件名称或描述" :disabled="disField"/> 
				</el-form-item>  
				...
				<el-form-item v-else-if="editForm.itemType=='8'" label="文本内容" prop="dvalues">
					<el-input type="textarea" rows=8 v-model="editForm.dvalues"  placeholder="文本内容" :disabled="disField"/>  
				</el-form-item>  
				<el-form-item v-else-if="editForm.itemType=='9'" label="默认图片链接" prop="dvalues">
					<el-input v-model="editForm.dvalues" style="width:40%;" placeholder="图片链接地址" :disabled="disField"/> 
					<el-input v-model="editForm.dnames"  style="width:40%;" placeholder="图片名称或描述" :disabled="disField"/> 
				</el-form-item>   
				...
				<el-form-item v-else-if="editForm.itemType=='13'" label="默认选中" prop="dvalues">
					<el-radio-group v-model="multipleDvalues" placeholder="默认选中" multiple :disabled="disField">
						<el-radio
				...
				<el-form-item v-else-if="editForm.itemType=='14'" label="默认选中" prop="dvalues">
					<el-checkbox-group v-model="multipleDvalues" placeholder="默认选中" :disabled="disField">
						<el-checkbox
					...
					<el-row> 
						<el-col :span="12">
							<el-form-item  label="值" prop="dvalues">
								<el-input v-model="editForm.dvalues"  placeholder="值" :disabled="disField"/> 
							</el-form-item>  
						</el-col>
						<el-col :span="12">
							<el-form-item  label="值名称" prop="dnames">
								<el-input v-model="editForm.dnames"   placeholder="值名称" :disabled="disField"/> 
							</el-form-item>  
						</el-col>
					</el-row> 
				</template>
				...
				<el-form-item label="备注" prop="remark">
					<el-input type="textarea" v-model="editForm.remark" placeholder="备注" :disabled="disField"/>
				</el-form-item>  
				<el-form-item label="" prop="required">
					<el-space wrap>
					排序:<el-input type="number" style="width:10%;" v-model="editForm.seq" :min="0" :max="999" :disabled="disField"/>
					 <el-checkbox v-model="editForm.required" true-label="1" false-label="0" :disabled="disField">必输项</el-checkbox> 
					 <el-checkbox v-model="editForm.isShow" true-label="1" false-label="0" :disabled="disField">显示</el-checkbox>  
					长度： <el-input type="number" style="width:10%;" v-model="editForm.itemSize" :min="0" :max="200" :disabled="disField"/>
					输入提示：<el-input style="width:10%;" v-model="editForm.inputFormat" placeholder="输入提示" :disabled="disField"/>
					</el-space>
				</el-form-item>   
				 
					<MdpExtInfosx  :cfgDisabled="false" v-model="editForm.extInfos" :disabled="disBtn('addBtn')"/> 
	...
	export default { 
		...
		methods: {  
			initOptions(itemCode){
				this.$mdp.ajaxGetDictOptions(itemCode,{},true).then(res=>{
