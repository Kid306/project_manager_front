...
const emit = defineEmits<{ (e: 'update:modelValue', v: string),(e: 'change', v: string) }>()
...
function handleClick({ props }) {
  ...
  emit('change',currentActiveType.value + iconList.value[currentActiveType.value][0])
...
function onChangeIcon(item) {
  ...
  emit('change',currentActiveType.value + item)
  visible.value = false
}
...
<template>
  <div class="selector">
    ...
    <ElInput v-if="showInput" v-model="inputValue" @click="visible = !visible">
      <template #append>
        <ElPopover
          :popper-options="{
            placement: 'auto'
