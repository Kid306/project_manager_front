      ...
      <el-col :offset="6" :span="16">
        ...
        >
          ...
          <el-form-item label="抄送人" prop="copyUserIds"> 
            <MdpSelectUser v-model="auditForms[index].copyUserIds"  multiple placeholder="请选择抄送人"/>
          </el-form-item>
        </el-form>
...
import * as DefinitionApi from '@/api/mdp/workflow/re/procdef'
import * as ProcessInstanceApi from '@/api/mdp/workflow/hi/procinst'
import * as TaskApi from '@/api/mdp/workflow/ru/task'
import ProcessInstanceBpmnViewer from './components/ProcinstBpmnViewer.vue'
import ProcessInstanceTaskList from './components/TaskList.vue'
import TaskReturnForm from './components/TaskRollbackForm.vue'
import TaskDelegateForm from './components/TaskDelegateForm.vue'
import TaskTransferForm from './components/TaskTransferForm.vue'
import TaskSignCreateForm from './components/TaskSignCreateForm.vue'
...
const userId = useUserStore().getUserInfo.userid // 当前登录的编号
...
const handleAudit = async (task, pass) => {
  ...
  if (pass) {
    ...
    await TaskApi.completeTask(data)
...
const getProcessInstance = async () => {
  try {
    ...
    const { data} = await ProcessInstanceApi.queryProcinstById({id:id})
    if (!data) {
    ...
    // 加载流程图
    bpmnXml.value = (
      await DefinitionApi.queryProcdefById({id:processDefinition.id})
    )?.bpmnXml
  } finally {
...
const getTaskList = async () => {
  ...
  try {
    ...
    const { data} = await TaskApi.listByProcInstId({procInstId:id})
    tasks.value = []
    // 1.1 移除已取消的审批
    data.forEach((task) => {
      if (task.status !== 4) {
