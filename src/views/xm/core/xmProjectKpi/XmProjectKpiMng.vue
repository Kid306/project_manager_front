<template>
		...
		<el-row   class="xm-kpi">
			...
			<el-table ref="table" v-adaptive="{bottom:50}" :data="xmProjectKpis" @sort-change="sortChange" highlight-current-row v-loading="load.list" border @selection-change="selsChange" @row-click="rowClick" style="width: 100%;">
				...
				<el-table-column prop="nextCalcDate" label="下次考核" min-width="80"  />
			...
			<mdp-dialog title="编辑xm_project_kpi" v-model="editFormVisible"  size="50%"  append-to-body   :close-on-click-modal="false">
					<xm-project-kpi-edit :xm-project-kpi="editForm" :visible="editFormVisible" @cancel="editFormVisible=false" @submit="afterEditSubmit" />
			</mdp-dialog>

			<!--新增 XmProjectKpi xm_project_kpi界面-->
			<mdp-dialog title="新增xm_project_kpi" v-model="addFormVisible"  size="50%"  append-to-body   :close-on-click-modal="false">
				<xm-project-kpi-add :xm-project-kpi="addForm" :visible="addFormVisible" @cancel="addFormVisible=false" @submit="afterAddSubmit" />
			</mdp-dialog>
		</el-row>
...
<script>
	...
	import  XmProjectKpiAdd from './XmProjectKpiAdd.vue';//新增界面
	import  XmProjectKpiEdit from './XmProjectKpiEdit.vue';//修改界面
