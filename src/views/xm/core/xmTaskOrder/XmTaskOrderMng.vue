<template>
		...
		<el-row>
			...
			<el-button v-loading="load.list" :disabled="load.list==true" @click="searchXmTaskOrders" icon="search">查询</el-button>
		...
		<el-row class="padding-top">
			...
			<el-table ref="xmTaskOrderTable" :data="xmTaskOrders" v-adaptive="{bottom:50}" @sort-change="sortChange" highlight-current-row v-loading="load.list" border @selection-change="selsChange" @row-click="rowClick" style="width: 100%;">
				...
				-->
				<el-table-column prop="id" label="订单编号" min-width="120" show-overflow-tooltip  fixed="left" />
		...
		<el-row>
			...
			<mdp-dialog title="编辑任务相关费用订单表" v-model="editFormVisible"  width="60%"  append-to-body   :close-on-click-modal="false">
			    <xm-task-order-edit op-type="edit" :xm-task-order="editForm" :visible="editFormVisible" @cancel="editFormVisible=false" @submit="afterEditSubmit" />
			</mdp-dialog>

			<!--新增 XmTaskOrder 任务相关费用订单表界面-->
			<mdp-dialog title="新增任务相关费用订单表" v-model="addFormVisible"  width="60%"  append-to-body  :close-on-click-modal="false">
			    <xm-task-order-edit op-type="add" :visible="addFormVisible" @cancel="addFormVisible=false" @submit="afterAddSubmit" />
			</mdp-dialog>
	    </el-row>
...
import { listXmTaskOrder, delXmTaskOrder, batchDelXmTaskOrder,editSomeFieldsXmTaskOrder } from '@/api/xm/core/xmTaskOrder';
import  XmTaskOrderEdit from './XmTaskOrderEdit.vue';//新增修改界面
