<template>
		...
		<el-space> 
				<xm-product-select title="选择更多产品加入项目" width="300px" show-style="origin" placeholder="选择更多产品加入项目" @change2="onProductSelect" :autoSelect="false" v-if="selProject && selProject.id"/>
 				<xm-project-select  title="选择更多项目加入产品" width="300px" show-style="origin" placeholder="选择更多项目加入产品" :autoSelect="false" @change2="onProjectSelect" v-if="xmProduct && xmProduct.id"/>
 
		</el-space> 
			<!--列表 XmProductProjectLink 产品与项目的关联关系表，一般由产品经理挂接项目到产品上-->
			<el-table ref="xmProductProjectLink"  :data="xmProductProjectLinks"  v-adaptive="{bottom:50}" @sort-change="sortChange" highlight-current-row v-loading="load.list" border @selection-change="selsChange" @row-click="rowClick" style="width: 100%;">
				...
				<el-table-column prop="seq" label="顺序" min-width="80" sortable>
					<template #default="scope">
				       {{scope.row.seq}}
                     <span class="cell-bar"><el-input style="display:inline;" title="0-999之间，数值越小越靠前" v-model="scope.row.seq" placeholder="" @change="editSomeFields(scope.row,'seq',$event)" :maxlength="22"/></span>
					</template>
				</el-table-column>
				<el-table-column prop="ctime" label="加入时间" min-width="80"  />
				<el-table-column prop="cusername" label="操作者" min-width="80"  />
 				<el-table-column label="操作" width="250" fixed="right">
					<template #default="scope">
						<el-button v-if="xmProduct && xmProduct.id" type="primary" @click="goToProject(scope.row,scope.$index)" icon="search" >进入项目</el-button>
						<el-button v-if="selProject && selProject.id" type="primary" @click="goToProduct(scope.row,scope.$index)" icon="search" >进入产品</el-button>

 						<el-button type="danger" @click="handleDel(scope.row,scope.$index)" icon="delete" v-loading="load.del">移出</el-button>
					</template>
				</el-table-column>
			</el-table>
			...
			getXmProductProjectLinks() {
				...
				listXmProductProjectLink(params).then((res) => {
					...
					if(tips.isOk){
						...
						this.xmProductProjectLinks = res.data;
					}else{
          ...
          editSomeFields(row,fieldName,$event){
            ...
            if(this.sels.length>0){
              if(!this.sels.some(k=> k.projectId==row.projectId &&  k.productId==row.productId)){
                ...
                params['$pks']=this.sels.map(i=>{ return { projectId:i.projectId,  productId:i.productId}})
            }else{
                params['$pks']=[row].map(i=>{ return { projectId:i.projectId,  productId:i.productId}})
            }
			...
			goToProject(row){
				this.$router.push({path:'/xm/project/view/overview',query:{projectId:row.projectId}})
			},
			goToProduct(row){
				this.$router.push({path:'/xm/product/view/overview',query:{productId:row.productId}})
			},
			/**end 自定义函数请在上面加**/
