<template>
		...
		<el-row v-if="!xmIteration" class="page-main">
			...
			<el-button type="primary" v-loading="load.list" :disabled="load.list==true" @click="searchXmIterationStates">查询</el-button>
  		</el-row>
		<el-row class="page-main ">
			<!--列表 XmIterationState 迭代定义-->
			<el-table ref="table" v-adaptive="{bottom:50}" :data="xmIterationStates" @sort-change="sortChange" highlight-current-row v-loading="load.list" border @selection-change="selsChange" @row-click="rowClick" style="width: 100%;">
 				<el-table-column sortable type="index" width="45" />
 				<el-table-column prop="iterationName" label="迭代名称" min-width="80"  />
				...
				<el-table-column prop="budgetCost" label="预算成本" min-width="80"  />
				<el-table-column prop="budgetWorkload" label="预算工作量" min-width="80"  />
				<el-table-column prop="distBudgetCost" label="已分配到任务的预算" min-width="80"  />
				<el-table-column prop="distBudgetWorkload" label="已分配到任务的预算工作量" min-width="80"  />
				<el-table-column prop="actCost" label="实际成本" min-width="80"  />
				<el-table-column prop="actWorkload" label="实际工作量" min-width="80"  />
				<el-table-column prop="actStaffNum" label="实际投入人员数" min-width="80"  />
				...
				<el-table-column prop="projectCnt" label="关联项目数" min-width="80"  />
				<el-table-column prop="productCnt" label="关联产品数" min-width="80"  />
				<el-table-column prop="menuCnt" label="关联需求数" min-width="80"  />
				<el-table-column prop="taskCnt" label="关联任务数" min-width="80"  />
				<el-table-column prop="taskFinishCnt" label="已完成的任务数" min-width="80"  />
			</el-table>
			...
			<mdp-dialog title="编辑迭代定义" v-model="editFormVisible"  size="50%"  append-to-body   :close-on-click-modal="false">
				  <xm-iteration-state-edit :xm-iteration-state="editForm" :visible="editFormVisible" @cancel="editFormVisible=false" @submit="afterEditSubmit" />
			</mdp-dialog>

			<!--新增 XmIterationState 迭代定义界面-->
			<mdp-dialog title="新增迭代定义" v-model="addFormVisible"  size="50%"  append-to-body  :close-on-click-modal="false">
				<xm-iteration-state-add :xm-iteration-state="addForm" :visible="addFormVisible" @cancel="addFormVisible=false" @submit="afterAddSubmit" />
			</mdp-dialog>
		</el-row>
...
<script>
	...
	import  XmIterationStateAdd from './XmIterationStateAdd.vue';//新增界面
	import  XmIterationStateEdit from './XmIterationStateEdit.vue';//修改界面
